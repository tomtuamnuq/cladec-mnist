...
def train_cladec(x, y, classifier: Sequential, layer_name: str, alpha: int, epochs: int, decoder: Model = None):
    ...
    for dataset, saved_models_path in zip(datasets, model_paths):
        ...
        for alpha in ALPHAS:  # [0]
            ...
            for layer_name in layers:  # ['my_dense']
                ...
                cladec = train_cladec(train_images, train_labels_c, classifier, layer_name, alpha, epochs, decoder)
                ...
                if alpha == 0:
                    ...
                    refae.save_weights(saved_models_path_refae.joinpath(layer_name).joinpath(layer_name))
...
def test_create_models():
    ...
    saved_models_path_refae = src.utils.SAVED_MODELS_PATH_MNIST.joinpath('refae').joinpath(DENSE_LAYER_NAME).joinpath(
        DENSE_LAYER_NAME)
    decoder = ClaDec.create_128_dense_decoder()
    cladec = ClaDec.create_from_weights(saved_models_path, classifier, DENSE_LAYER_NAME, alpha / 100, decoder)
